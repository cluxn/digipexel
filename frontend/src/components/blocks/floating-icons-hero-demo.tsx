...
// Default icon slots — official brand SVGs
// OpenAI served from local /openai.svg (simpleicons CDN has no openai slug)
  ...
  useEffect(() => {
    async function fetchHero() {
      ...
      if (json.status === "success" && json.data) {
        const { iconSlots: slots, ...text } = json.data as { iconSlots?: typeof FALLBACK_ICON_SLOTS } & typeof FALLBACK_HERO_TEXT;
      ...
      try {
        ...
        if (local) {
          ...
          setHeroText({ ...FALLBACK_HERO_TEXT, ...text });
          // Detect stale slots (old string-key format or broken CDN URLs) and upgrade
          if (Array.isArray(slots) && slots.length > 0) {
            const isStale =
              ...
              // Slot 2 used to point to cdn.simpleicons.org/openai which returns 404
              slots.some((s: { icon: string }) =>
                s.icon.includes("simpleicons.org/openai")
              );
            if (isStale) {
              // Upgrade: keep custom text edits, replace icon slots with correct defaults
              const upgraded = { ...data, iconSlots: FALLBACK_ICON_SLOTS };
              localStorage.setItem("PREVIEW_hero", JSON.stringify(upgraded));
              setIconSlots(FALLBACK_ICON_SLOTS);
            } else {
              setIconSlots(slots);
            }
          } else {
          ...
          // Seed localStorage with CDN-URL defaults so admin panel shows them
          localStorage.setItem("PREVIEW_hero", JSON.stringify({
            ...FALLBACK_HERO_TEXT,
            iconSlots: FALLBACK_ICON_SLOTS,
          }));
        }
      } catch {}
...
export function Navbar({ className, darkHero = true }: { className?: string; darkHero?: boolean }) {
  ...
  useEffect(() => {
    async function fetchNav() {
      ...
      if (json.status === "success" && json.data) {
        const nd = json.data as { ctaText?: string; ctaHref?: string };
        setNavCta({ text: nd.ctaText ?? "Book a Call", href: nd.ctaHref ?? "/contact-us" });
      }
    }
    fetchNav().catch(() => {});
  }, []);
  ...
  useEffect(() => {
    safeFetch(`${API_BASE_URL}/settings.php`).then(json => {
      if (json?.status === "success" && json.data) {
        const sd = json.data as { calendly_url?: string };
        if (sd.calendly_url) setCalendlyUrl(sd.calendly_url);
      }
    }).catch(() => {});
  }, []);
